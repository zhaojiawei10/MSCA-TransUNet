...
class MSCF(nn.Module):
    def __init__(self, in_channels1, in_channels2, out_channels):
        ...
        self.conv1x1_1 = nn.Conv2d(in_channels1, out_channels, kernel_size=1)
        self.conv1x1_2 = nn.Conv2d(in_channels2, out_channels, kernel_size=1)
        self.conv3x3 = nn.Conv2d(out_channels * 2, out_channels, kernel_size=3, padding=1)
        self.conv5x5 = nn.Conv2d(out_channels * 2, out_channels, kernel_size=5, padding=2)
        self.relu = nn.ReLU()
    ...
    def forward(self, x1, x2):
        ...
        x = self.conv3x3(x) + self.conv5x5(x)
        return self.relu(x)

class CAAB(nn.Module):
    def __init__(self, in_channels, reduction=16):
        super(CAAB, self).__init__()
    ...
    def forward(self, x1, x2):
        if self.conv1x1 is None:
            self.conv1x1 = nn.Conv2d(in_channels=x2.size(1), out_channels=x1.size(1), kernel_size=1).to(x2.device)
        x2 = self.conv1x1(x2)
        x = x1 + x2
        b, c, _, _ = x.size()
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        self.mscf = MSCF(in_channels1=head_channels, in_channels2=head_channels, out_channels=head_channels)
        self.caab = nn.ModuleList([
            CAAB(in_channels=out_ch) for out_ch in out_channels
        ])
